...
이 문제는 문자열 `rny_string`에 들어 있는 모든 `'m'` 문자를 `"rn"`으로 바꾸면 되는 문자열 치환 문제입니다.

자바에서는 문자열을 쉽게 바꿀 수 있도록 `replaceAll()` 메서드를 제공합니다.  
`replaceAll("m", "rn")`을 사용하면 문자열 안에 있는 모든 `"m"`이 `"rn"`으로 한 번에 변경된 새로운 문자열이 반환됩니다.

따라서 별도로 반복문을 돌면서 문자를 하나씩 확인하지 않아도, 문자열 치환 메서드 하나만으로 간단하게 해결할 수 있습니다.
